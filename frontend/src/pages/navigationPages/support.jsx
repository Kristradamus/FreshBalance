...
useEffect(() => {
  const topicMap = {};
  Object.values(supportData || {}).flat().forEach(topic => {
    if (topic && topic.issue) {
      const id = encodeURIComponent(topic.issue.toLowerCase().replace(/\s+/g, '-'));
      topicMap[id] = topic;
    }
  });
    ...
    <div className="support">
      ...
      <div className="supportContent">
        ...
        <div className="supportMain">
          {currentTitle && (<button className="supportBackButton" onClick={handleBack}>{t("support.backToSupport")}</button>)}
          <h1><strong>{currentTitle || t("support.welcome")}</strong></h1>
