      ...
      <div className={`subPlan sub${plan.type}`}>
        <div className={`subPlanTop sub${plan.type}Top`}>
          ...
          <div className="subButtons">
            <button className={`subTryFree ${plan.type}TryFree`} onClick={() => handleTryFree(plan.type)}>
              <strong>{t("subscriptions.tryFree")}</strong>
            </button>
            <button className={`subBuyNow ${plan.type}BuyNow`} onClick={() => handleBuyNow(plan.type)}>
              <strong>{t("subscriptions.buyNow")}</strong>
          ...
          <ul className={`subBenefits sub${plan.type}Benefits`}>
            {plan.benefits.map((item, index) => (
              <li key={index} className="subBenefit">
                <strong className="subBenefitTitle">
    ...
    <div className="subscriptions">
      <div className="subBox">
        <div className="subToggleContainer" onClick={subHandleToggle}>
          ...
          <p className={`subToggleText ${!isYearly ? "active" : ""}`}>
            <strong>{t("subscriptions.monthly")}</strong>
          </p>
          <p className={`subToggleText ${isYearly ? "active" : ""}`}>
            <strong>{t("subscriptions.yearly")}</strong>
          </p>
        </div>
