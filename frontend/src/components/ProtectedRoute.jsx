  ...
  <div className="authAlertContainer">
    <div className="authAlertBox">
      ...
      <div className="authAlertButtons">
        <button className="authAlertBtn" onClick={() => navigate('/email-check', {state: { from: location.pathname }})}>{t("protectedRoute.login")}</button>
        <button className="authAlertCancelBtn" onClick={() => {navigate(-2);}}>{t("protectedRoute.cancel")}</button>
      </div>
    </div>
  </div>
);};
